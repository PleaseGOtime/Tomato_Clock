...
const TimerUP = {
  ...
  stop() {
    ...
    this._reset(); this._syncUI(); this._save(); NativeBridge.stopTimer();
  },
  reset() { if(this.state==='running'){this.accumulated+=(Date.now()-this.startTime)/1000;this._stopTick();}const e=Math.floor(this.accumulated);if(e>0&&this.state!=='idle'){const act=document.getElementById('up-activity-input').value.trim()||'未记录';DB.addRecord({id:Date.now(),date:today(),startTime:Math.floor((this.startTime||Date.now()-e*1000)/1000),endTime:Math.floor(Date.now()/1000),duration:e,activity:act});showToast('已保存：'+fmtDuration(e));renderStats();} this._reset(); this._syncUI(); this._save(); NativeBridge.stopTimer(); },
